...
async function getBeerJSON() {
    try {
        ...
        console.error(err);
        makeAlert('No beer review found!')
    }
}

async function chartBeer() {
        
    const beerData = await getBeerJSON();
    if (!beerData) return;
    const beerAttributes = Object.keys(beerData.beer.attributes).map(attr => {
        return capitalizeFirst(attr).replace('_', ' ');
    });
    const radarChartCanvas = document.getElementById('radar');
    ...
    makeChart(radarChartCanvas, "radar", beerChartData, beerChartOptions);
}

window.onload = chartBeer;